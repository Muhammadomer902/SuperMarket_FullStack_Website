...
const CategoryPage = async ({params}:Props) => {
    ...
    return (
        <div className="flex flex-wrap text-sky-800">
            {
                products.map(item=>(
                    <Link className="w-full h-[60vh] border-l-2 border-t-2 border-r-2 border-b-2 border-sky-600 sm:w-1/2 lg:w-1/3 p-4 flex flex-col justify-between group hover:bg-fuchsia-200 even:bg-fuchsia-100" href={`/product/${item.id}`} key={item.id}>
                        {/* Image Container */}
                        {
                            item.img && 
                            (
